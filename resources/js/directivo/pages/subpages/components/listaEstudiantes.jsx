...
const ListaEstudiantes = () => {
    ...
    const cargarEstudiantes = (datos) => {
        setCargando(true)
        axios.get('/api/directivo/estudiantes/', {
            params: datos
        })
        .then(({ data }) => {
            if(data.data === undefined){
                setEstudiantes({data: data, total:data.length});
            }
            else
                setEstudiantes(data);
        })
    ...
    const eliminar = () => {
        axios.delete('/api/directivo/estudiantes/' + idEliminar)
        .then(res => {
    ...
    return <div>
        ...
        <div className="table-responsive" style={{height:'60vh'}}>
            ...
            { estudiantes.data !== undefined  && <ModalConfirmar id="deleteEst" text={'¿Quieres eliminar a ' + nombreEliminar + '?'} eliminar={true} confirmar={eliminar}/>}
        </div>
        }
